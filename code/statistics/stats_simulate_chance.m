% pconf = stats_simulate_chance ( ntrials, alpha[, nsims] )
%
...
%
% Out:
%   dChanceAcc - Threshold of chance accuracy for given parameters. 
%
%   iNumTrials - Number of minium correct classifications to achieve significantly better than
%                chance performance. 
%
...
function [dChanceAcc, iNumTrials, pconf] = stats_simulate_chance(ntrials, alpha, nsims)
...
for s = 1:nsims
    ...
    for t = 1:length(ntrials)
      ...
      X = [X, repmat(t, 1, ntrials(t))]; 
    
    end
    
    % getting random prediction array
    Y = X(randperm(length(X)));
%     Y = randi(numel(ntrials),size(X));
